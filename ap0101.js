...
function init() {
  ...
  //光源なし
  //const box1Material = new THREE.MeshBasicMaterial({ color: 0x008060});
  //光源が適用されるように
  ...
  // 箱3の表示設定 (自由な箱)====================
  const box3Geometry = new THREE.BoxGeometry(3, 6, 5);
  //光源が適用されるように
  const box3Material = new THREE.MeshLambertMaterial({ color: 0x198002});
  ...
  camera.position.x = 30;//25
  camera.position.y = 18;//15
  camera.position.z = 13;//30
  camera.lookAt(0, 4, 0);//カメラの場所の座標
